...
const Section: React.FC<{title: string; children: React.ReactNode; className?: string}> = ({title, children, className}) => (
    <div className={`py-6 border-b border-gray-200 last:border-b-0 ${className}`}>
            ...
            <div className="md:grid md:grid-cols-12">
                ...
                <div className="px-4 pb-4 md:p-0 md:col-span-10 md:divide-y md:divide-gray-200">
                    {items.map((item, index) => (
                        <div key={item.id} className="py-2 md:py-0 md:grid md:grid-cols-10 md:gap-4 items-center md:p-3 border-b md:border-b-0 last:border-b-0">
        ...
        <div className="border border-gray-300 rounded-xl divide-y divide-gray-300 text-sm">
            <div className="grid grid-cols-1 lg:grid-cols-2 lg:divide-x lg:divide-gray-300">
                <div className="p-4 space-y-2">
                    ...
                    <Field label="서명" value={data.applicantSignature ? <img src={data.applicantSignature} alt="applicant signature" className="h-20 bg-gray-100 border rounded-md p-1"/> : '서명 없음'} />
                </div>
                <div className="p-4 space-y-2 border-t lg:border-t-0 border-gray-300">
                    ...
                    <Field label="서명" value={data.managerSignature ? <img src={data.managerSignature} alt="manager signature" className="h-20 bg-gray-100 border rounded-md p-1"/> : '서명 없음'} />
                </div>
            </div>
            ...
            {(data.gasMeasurements && data.gasMeasurements.length > 0 && data.gasMeasurements.some(g => g.name)) && (
                 <div className="p-4">
                    <Field label="가스농도 측정" value={
                        <div className="w-full text-center border rounded-lg overflow-hidden">
  ...
  return (
    <Card ref={ref}>
      ...
      <div className="space-y-4">
        ...
        <Section title="안전 서약서" className="border-b-0 pb-0">
            <Field label="성명" value={data.safetyPledge.name} />
            <Field label="전체 동의" value={data.safetyPledge.agreeToAll ? '예' : '아니오'} />
            <Field label="서명" value={data.safetyPledge.signature ? <img src={data.safetyPledge.signature} alt="signature" className="h-20 bg-gray-100 border rounded-md p-1"/> : '서명 없음'} />
        </Section>
      </div>
    </Card>
  );
